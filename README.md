...
Метод __stopIndexing__, помеченный аннотацией @GetMapping("/stopIndexing"), вызыает метод  __stopIndexingInController__, 
...
которые нужно проиндексировать. В каждом потоке запускается метод __run__ класса __Task__ расположенного
в папке __auxiliary__, который наследует интерфейс __Runnable__. В данном классе кроме метода __run__ содержатся методы __writeInSql__ и 
...
являются статическими. Основные из них: __amountOfLemmas__ и __writeLemmasAndIndexIntoSql__.
...
Методы __writeLemmasIntoSql__ и __writeIndexIntoSql__ перебирают в цикле список страниц для каждого сайта (сайт и список
страниц передаются в аргементы метода). Метод __writeLemmasIntoSql__  во вложенном цикле, перебирающем мар возвращаемый
методом __amountOfLemmas__, при каждой итерации создается объект класса __Lemma__  в конструктор которого
передаются объект класса Site, передаваемый в аргументы метода, значение String s являющееся ключом
из МАР, возвращаемой методом __amountOfLemmas__ и 1 в качестве аргумента __frequency__. Далее
объект записывается в TreeMap где ключом является строка, состоящая из имени сайта и значения __s__.
и в качестве значения объект класса __Lemma__. При совпадении ключей значение перезаписывается со значением 
frequency + 1. Затем значения данной МАР при помощи метода __saveAll__ репозитория LemmaRepository
записываются в базу данных. Аналогичный перебор происходит в __writeIndexIntoSql__ только здесь во вложенном 
цикле создается объект класса __SearchIndex__ в конструктор которого передается объект класса __Page__
из списка страниц, id объекта класса __Lemma__, получаемый при помощи метода __getIdBySiteIdAndLemma__
рупозитория __LemmaRepository__ и значение integer в качестве аргумента __lemmaRank__. Далее объект отправляется в 
arrayList __searchIndexList__ значения которого после записываются в базу данных при помощи метода __saveAll__
репозитория __SearchIndexRepository__. Методы __writeLemmasIntoSql__ и __writeIndexIntoSql__ поочередно
выполняются в методе __writeLemmaAndIndexIntoSql__
