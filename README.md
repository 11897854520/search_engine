...
#### Вспомогательные классы (папка __auxiliary__).
...
(для таблиц __lemma__ и __search_index__) которые передаются в метод __insert__ репозитория
__InsertRepository__. В поле rank таблицы __search_index__ передаются значения МАРы возвращаемой
