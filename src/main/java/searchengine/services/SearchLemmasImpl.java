...
public class SearchLemmasImpl implements SearchLemmas {
    ...
    private List<Lemma> listOfLemmas(String text, String url) throws IOException {
        ...
        contentHandling.amountOfLemmas(text).keySet().forEach(s -> {
            ...
            if (lemmaRepository.findBySiteIdAndByLemma(siteRepository.findByUrl(url).getId()
                    , s) != null) {

                lemmas.add(lemmaRepository.findBySiteIdAndByLemma(siteRepository.findByUrl(url).getId()
                        , s));
    ...
    private List<Lemma> shortedListOfLemmas(List<Lemma> listOfLemmas) {
        ...
        return listOfLemmas.stream()
                .filter(lemma -> lemma.getFrequency()
                        <= ((lemmaRepository.findMaxFrequencyBySiteId(lemma.getSite().getId()) > 100
                        ? lemmaRepository.findMaxFrequencyBySiteId(lemma.getSite().getId()) * 70 / 100
                        : lemmaRepository.findMaxFrequencyBySiteId(lemma.getSite().getId()))))
    ...
    private Map<Lemma, List<Page>> getListOfPages(List<Lemma> shortedListOfLemmas) {
        ...
        shortedListOfLemmas.forEach(lemma -> {
            ...
            List<Page> pages = searchIndexRepository.getAllSearchIndexByLemma(lemma.getId()).stream()
        ...
        getListOfPages.values().stream().flatMap(Collection::parallelStream)
                .forEach(page -> {
                    ...
                    getListOfPages.keySet().forEach(lemma -> {
                        ...
                        if (searchIndexRepository.getRankByLemmaAndPage(lemma.getId(), page.getId()) != null) {

                            listOfRelevance.add(searchIndexRepository.getRankByLemmaAndPage(lemma.getId()
                                    , page.getId()));
    ...
    public InformationAboutSearching searchResult(String query, String site, int offset, int limit)
            throws IOException {
